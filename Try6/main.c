...
void draw(GLuint vbo, float x, float y, float rot) {
  ...
  glUniform1f(uniform_m_time, glfwGetTime());
...
int main(void)
{
  ...
  uniform_m_time = glGetUniformLocation(program, "m_time");
  if (uniform_m_transform == -1) {
    fprintf(stderr, "Could not bind uniform m_time");
    return 0;
  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (float i = -1.0f; i < 1.0f; i += 0.1) {
      draw(vbo_shape2, i, 0.0f, i + angle / 10.0f);
    }
